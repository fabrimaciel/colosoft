...
var Skinny_FormID = ""{1}"";
			...
			page.ClientScript.RegisterClientScriptBlock(typeof(Manager), typeof(Manager).FullName, pageScript, false);
		}
		...
		bool CheckIfRedirectedToLoginPage()
		{
			...
			if(returnURL != null && returnURL.Length > 0)
			{
				...
				if(!string.IsNullOrEmpty(req.Params["SCB"]) && req.Params["SCB"] == "1")
				{
					HttpResponse resp = context.Response;
			...
			foreach (ParameterInfo paramInfo in pis)
			{
				...
				if(paramValue != null)
				{
					if(paramInfo.ParameterType.IsArray)
					{
						...
						{
							array.SetValue(Convert.ChangeType(HttpUtility.UrlDecode(values[index]), type), index);
						}
						param = array;
					}
					else
					{
						param = Convert.ChangeType(HttpUtility.UrlDecode(paramValue), paramInfo.ParameterType);
					}
				}
				parameters[i] = param;
				++i;
