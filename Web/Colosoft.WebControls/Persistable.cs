...
namespace Colosoft.WebControls
{
	...
	public class Persistable<T> : IDisposable where T : class, new()
	{
			...
			get
			{
				lock (SyncRoot)
				{
					if(this.IsEmpty)
					{
						if(this.AutoLoad)
							Load();
						if(_value == null && this.AutoCreate)
							_value = new T();
					}
					return _value;
				}
		...
		public Persistable(string file) : this()
		{
			this._file = file;
		}
		...
		{
			string file = this._file;
			HttpRuntime.Cache.Insert(file, new object(), new CacheDependency(file), Cache.NoAbsoluteExpiration, Cache.NoSlidingExpiration, CacheItemPriority.High, new CacheItemRemovedCallback(NotifyFileChanged));
		}
