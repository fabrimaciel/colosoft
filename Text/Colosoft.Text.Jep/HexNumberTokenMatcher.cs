...
namespace Colosoft.Text.Jep.ConfigurableParser.Matchers
{
	...
	internal class HexNumberTokenMatcher : RegExpTokenMatcher
	{
		[NonSerialized]
		private INumberFactory nf;

		public HexNumberTokenMatcher() : base("0x[0-9a-fA-F]+")
		{
		}
		...
		public override Token BuildToken(string s)
		{
			try
			{
				long num = long.Parse(s.Substring(2));
				return new NumberToken(s, this.nf.CreateNumber(num));
			}
