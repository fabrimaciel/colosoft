...
namespace Colosoft.Text.Jep.Functions
{
	...
	public class Imaginary : PostfixMathCommand
	{
		public Imaginary()
		{
		...
		public JepDouble Im(object param)
		{
			if(param is Complex)
			{
				return new JepDouble(((Complex)param).Im);
			}
			if(!(param is JepDouble))
			{
				throw new EvaluationException("Invalid parameter type");
			}
			return new JepDouble(0.0);
		}
