...
namespace Colosoft.Text.Jep.Functions
{
	...
	public class Real : PostfixMathCommand
	{
		public Real()
		{
		...
		public JepDouble Re(object param)
		{
			if(param is Complex)
			{
				return new JepDouble(((Complex)param).Re);
			}
			if(!(param is JepDouble))
			{
				throw new EvaluationException("Invalid parameter type");
			}
			return (JepDouble)param;
		}
