...
namespace Colosoft.Text.Jep.ConfigurableParser.Matchers
{
	...
	public class MultiLineMatcher : ITokenMatcher
	{
		...
		private ITokenMatcher startMatcher;

		public MultiLineMatcher(ITokenMatcher startMatcher, ITokenMatcher endMatcher, ITokenBuilder completeMatcher)
		{
			this.startMatcher = startMatcher;
			this.endMatcher = endMatcher;
			this.completeMatcher = completeMatcher;
		}
		...
		{
			if(this.startMatcher.Match(s) != null)
			{
				return new MultiLineToken(s, this.endMatcher, this.completeMatcher);
			}
			return null;
		}
	}
