...
{
	/// <summary>
	/// Implementação do substituto de serialização do vetor de <see cref="Guid"/>.
	/// </summary>
	internal sealed class GuidArraySerializationSurrogate : ContextSensitiveSerializationSurrogate
	{
		...
		{
			Guid[] guidArray = (Guid[])graph;
			for(int i = 0; i < guidArray.Length; i++)
				guidArray[i] = reader.ReadGuid();
			return guidArray;
		}
		...
		{
			Guid[] guidArray = (Guid[])graph;
			writer.Write(guidArray.Length);
			for(int i = 0; i < guidArray.Length; i++)
			{
				writer.Write(guidArray[i]);
			}
		}
	}
}
