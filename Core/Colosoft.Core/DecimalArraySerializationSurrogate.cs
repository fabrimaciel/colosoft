...
{
	/// <summary>
	/// Implementação do substituto de serialização do vetor de <see cref="decimal"/>.
	/// </summary>
	internal sealed class DecimalArraySerializationSurrogate : ContextSensitiveSerializationSurrogate
	{
		...
		{
			return new decimal[reader.ReadInt32()];
		}

		public override object ReadDirect(CompactBinaryReader reader, object graph)
		{
			decimal[] numArray = (decimal[])graph;
			for(int i = 0; i < numArray.Length; i++)
				numArray[i] = (decimal)CompactBinaryFormatter.Deserialize(reader, reader.CacheContext, false);
			return numArray;
		}
		...
		public override void WriteDirect(CompactBinaryWriter writer, object graph)
		{
			...
			for(int i = 0; i < numArray.Length; i++)
				CompactBinaryFormatter.Serialize(writer, numArray[i].ToString(), writer.CacheContext);
		}
	}
}
