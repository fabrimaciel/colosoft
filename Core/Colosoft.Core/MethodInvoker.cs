...
namespace Colosoft.Reflection
{
	...
	public class MethodInvoker
	{
		...
		public void SetDefaultValue(string parameterName, object value)
		{
			...
			throw new ConfigException("Method does not have a parameter named " + parameterName);
		}
		...
		public MethodInvokable PrepareInvoke(Hashtable parameters)
		{
			...
			for(int i = 0; i < _parameterInfos.Length; i++)
			{
				...
				if(pi.GetCustomAttributes(typeof(Colosoft.Configuration.Attributes.ElementContentAttribute), false).Length > 0)
				{
		...
		public object Invoke(object target, object[] parameterValues)
		{
			try
			{
				return MethodInfo.Invoke(target, Reflection.ReflectionFlags.InstanceCriteria, null, parameterValues, null);
			}
			catch(Exception e)
			{
				throw e;
		...
		public object Invoke(object target, Hashtable parameters)
		{
			...
			{
				throw new ConfigException("Unable to invoke method using given parameters.");
			}
		}
