...
using Colosoft.Configuration.Targets;
using Colosoft.Configuration.Handlers;
using Colosoft.Configuration.Exceptions;
...
namespace Colosoft.Configuration
{
	internal class ConfigurationMap
	{
		...
		{
			List<MemberAttributeInfo> memberInfos = LoadMembersConfigurationAttributes(Reflection.ReflectionFlags.InstanceCriteria, type);
			foreach (MemberAttributeInfo mai in memberInfos)
			{
				...
				case MemberTypes.Property:
					...
					break;
				default:
					throw new ConfigException("Unknown configuration target type for member {0} on class {1}.", mai.MemberInfo.Name, type);
				}
			}
			memberInfos = LoadMembersConfigurationAttributes(Reflection.ReflectionFlags.StaticCriteria, type);
			foreach (MemberAttributeInfo mai in memberInfos)
			{
				ConfigurationAttribute ca = mai.Attributes[0] as ConfigurationAttribute;
				switch(mai.MemberInfo.MemberType)
		...
		private void VerifyKeyPresence(ElementTarget target, bool keyFound)
		{
			...
			{
				throw new Colosoft.Configuration.Exceptions.MissingConfigurationKeyException(target.XmlNodePath);
			}
		}
	}
}
