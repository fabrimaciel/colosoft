...
{
	/// <summary>
	/// Implementação para calculo do CRC32.
	/// </summary>
	public class Crc32
	{
		...
		{
			System.Diagnostics.Debug.Assert((buffer != null) && (offset >= 0) && (length >= 0) && (offset <= buffer.Length - length), "check the caller");
			crc32 ^= 0xffffffffU;
			while (--length >= 0)
			{
		...
		/// this as the 'crc32' to compute the next chunk.  
		/// </summary>  
		public static uint Calculate(uint crc32, IEnumerator<byte> buffer)
		{
			crc32 ^= 0xffffffffU;
			while (buffer.MoveNext())
			{
				crc32 = crcTable[(crc32 ^ buffer.Current) & 0xFF] ^ (crc32 >> 8);
			}
			crc32 ^= 0xffffffffU;
			return crc32;
		}
