...
namespace Colosoft.Configuration
{
	...
	public class Configurator
	{
		...
		public static bool RefreshHandlers()
		{
			...
			{
				bool changed = false;
				foreach (BaseSectionHandler handler in handlers)
		...
		/// </summary>
		/// <returns>True caso os dados tenham sido alterados</returns>
		public static bool RefreshHandlers<HandlerType>()
		{
			lock (refreshLock)
			{
				bool changed = false;
				foreach (BaseSectionHandler handler in handlers)
					if(handler is HandlerType && handler.IsChanged)
					{
						changed = true;
						handler.Refresh();
					}
		...
		/// </summary>
		/// <returns></returns>
		public static bool RefreshHandlers(Predicate<BaseSectionHandler> predicate)
		{
			lock (refreshLock)
			{
				bool changed = false;
				foreach (BaseSectionHandler handler in handlers)
					if((predicate == null || predicate(handler)) && handler.IsChanged)
					{
						changed = true;
						handler.Refresh();
					}
