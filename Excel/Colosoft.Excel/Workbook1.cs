...
namespace Colosoft.Excel.ExcelXml
{
	...
	public partial class ExcelXmlWorkbook
	{
		...
		{
			return _Worksheets.FindIndex(sheet => sheet.Name.Equals(sheetName, StringComparison.OrdinalIgnoreCase));
		}
		...
		/// <returns><see cref="Colosoft.Excel.ExcelXml.Worksheet"/> reference to the requested sheet</returns>
		...
		{
			return _Worksheets.Find(sheet => sheet.Name.Equals(sheetName, StringComparison.OrdinalIgnoreCase));
		}
		...
		internal void ExportNamedRanges(XmlWriter writer, Worksheet ws)
		{
			...
			foreach (NamedRange nr in NamedRanges)
			{
				if(nr.Worksheet == ws)
				{
					...
					writer.WriteAttributeString("ss", "RefersTo", null, nr.Range.NamedRangeReference(true));
