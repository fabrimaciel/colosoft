...
{
	/// <summary>
	/// Classe com método para auxiliar no dados do protocolo HTTP.
	/// </summary>
	static class HttpUtil
	{
		...
		/// <summary>
		/// Decodifica o conteúdo da Url para o coleção de nomes de valores.
		/// </summary>
		...
		public static string UriDecode(string value, Encoding encoding)
		{
			...
			{
				if(value[i] == '%' && i < value.Length - 2 && IsHex(value[i + 1]) && IsHex(value[i + 2]))
				{
		...
		/// <summary>
		/// Verifica se o valor informado é um hexadecimal.
		/// </summary>
		/// <param name="value"></param>
		/// <returns></returns>
		...
		/// <summary>
		/// Executa o trim em todas as partes informadas.
		/// </summary>
		...
		/// <summary>
		/// Decodifica o valor HTML informado.
		/// </summary>
		/// <param name="value"></param>
		/// <returns></returns>
			...
			for(int i = 0; i < value.Length; i++)
			{
				if(value[i] == '&' && value.Length > i + 2)
				{
					...
					if(value[i + 1] == '#')
					{
						...
						{
							if(!(Char.IsDigit(value[j]) || (isHexNumeric && HttpUtil.IsHex(value[j]))))
							{
								isNumeric = false;
								break;
							}
