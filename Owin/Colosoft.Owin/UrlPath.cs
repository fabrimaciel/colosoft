...
{
	/// <summary>
	/// Classe com métodos para auxiliar na manipulação de urls.
	/// </summary>
	static class UrlPath
	{
		...
		internal static void CheckValidVirtualPath(string path)
		{
			...
			{
				throw new HttpException(string.Format("Physical path '{0}' not allowed.", path));
			}
			...
			if(HasScheme(path))
			{
				throw new HttpException(string.Format("Invalid vpath '{0}'", path));
			}
		}
		...
		internal static string GetDirectory(string path)
		{
			...
			{
				throw new ArgumentException("Empty path has no directory");
			}
			if((path[0] != '/') && (path[0] != '~'))
			{
				throw new ArgumentException("Path '{0}' must be rooted", path);
			}
			if(path.Length == 1)
			{
				return path;
			...
			if(num < 0)
			{
				throw new ArgumentException(string.Format("Path '{0}' must be rooted", path));
			}
		...
		internal static bool IsPathOnSameServer(string absUriOrLocalPath, Uri currentRequestUri)
		{
			...
			if(!Uri.TryCreate(absUriOrLocalPath, UriKind.Absolute, out uri))
			{
				...
				return !absUriOrLocalPath.TrimStart(new char[] {
					' '
		...
		internal static string MakeRelative(string from, string to)
		{
			...
			if(!IsRooted(from))
			{
				throw new ArgumentException(string.Format("Path '{0}' must be rooted", from));
			}
			if(!IsRooted(to))
			{
				throw new ArgumentException(string.Format("Path_must_be_rooted", new object[] {
					to
				}));
			}
			string str = null;
		...
		internal static string MakeVirtualPathAppAbsoluteReduceAndCheck(string virtualPath)
		{
			...
			{
				throw new ArgumentException(string.Format("Invalid app VirtualPath '{0}'", virtualPath));
			}
			return str;
		}
			...
			do
			{
				...
				if((((startIndex - num3) <= 3) && ((startIndex < 1) || (path[startIndex - 1] == '.'))) && (((num3 + 1) >= length) || (path[num3 + 1] == '.')))
				{
					if((startIndex - num3) == 3)
					{
						if(list.Count == 0)
						{
							throw new HttpException("Cannot exit up top directory");
						}
