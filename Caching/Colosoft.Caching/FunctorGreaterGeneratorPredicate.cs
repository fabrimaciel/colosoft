...
{
	/// <summary>
	/// Implementação do predicado da comparação maior.
	/// </summary>
	public class FunctorGreaterGeneratorPredicate : Predicate, IComparable
	{
		...
		public FunctorGreaterGeneratorPredicate(IFunctor functor, IGenerator generator)
		{
			_functor = functor;
		...
		public int CompareTo(object obj)
		{
			...
			{
				FunctorGreaterGeneratorPredicate predicate = (FunctorGreaterGeneratorPredicate)obj;
		...
		internal override void Execute(QueryContext queryContext, Predicate nextPredicate)
		{
			...
			{
				var data = memberFunction.GetData(store, queryContext.AttributeValues, base.Inverse ? ComparisonType.LESS_THAN_EQUALS : ComparisonType.GREATER_THAN, _generator.Evaluate(queryContext.AttributeValues));
